...
def get_access_points_linux() -> Values:
    cmd = [f"iwlist", "wlan0", "scanning" if posix_is_root() else "scan"]
    rows = subprocess.run(
...
class WifiInstrument(Instrument):
    def __init__(self):
        ...
        if platform == "linux" or platform == "linux2":
            ...
            do_measure_function = get_access_points_linux
        elif platform == "darwin":
            if not posix_is_root():
                print(
                    "WARNING: YOU MUST BE ROOT TO ACCESS CERTAIN INFORMATION SUCH AS THE BSSID (MAC) OF AN ACCESS POINT.")
            do_measure_function = get_access_points_macos
        elif platform == "win32":
            do_measure_function = get_access_points_windows
        else:
            raise Exception(f"Invalid platform: {platform}")
        super().__init__("WiFi", {"platform": platform}, [
